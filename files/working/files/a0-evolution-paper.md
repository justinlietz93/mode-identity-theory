...
The framework's CMB prediction therefore coincides with ΛCDM's, conditional on the structural decoupling: $a_0(z)$ governs the edge sector ($n=1$, $\Omega_H$), while recombination physics is a space-mode phenomenon ($n=3$, $\Omega_\Lambda$) inheriting ΛCDM's predictions for the acoustic peaks, polarization spectra, lensing reconstruction, and integrated Sachs-Wolfe imprint. The success of ΛCDM at fitting Planck 2018 [20] to sub-percent precision is consistent with this assignment, provided the edge/space decoupling is exact; the formal demonstration of the equivalence at the perturbation-theory level is the open work flagged below.
...
Relativistic MOND extensions address the same recombination-scale problem by adding fields or metrics (TeVeS [21], BIMOND [22], AeST [23]) whose dynamics reproduce MOND galactically while recovering GR-like behavior at cosmological scales; the present framework's selection rule provides a structurally distinct approach by assigning galactic-edge and cosmological-perturbation physics to different sectors of the same scaling-law postulate, with the empirical bound $\varepsilon \lesssim 10^{-5}$ confirming consistency with Planck without committing to a specific relativistic completion.

The remaining open question is a first-principles perturbation-theoretic derivation of the edge/space-mode decoupling at recombination-relevant scales. Concretely, the calculation must (i) derive the mode-projected Boltzmann propagator for cosmological perturbations under the $(n=3, \Omega_\Lambda)$ space-mode assignment, working from the linearized scaling law on $S^3$ with anti-periodic boundary conditions on the Möbius surface; (ii) compute the cross-coupling term between the edge-mode ($n=1$, $\Omega_H$) and space-mode ($n=3$, $\Omega_\Lambda$) sectors in the linearized propagator at recombination scales ($k \lesssim k_\text{eq}$), and identify the structural condition under which it vanishes rather than being tuned away; and (iii) verify that the resulting space-mode perturbation equations reduce to the standard ΛCDM linearized Einstein-Boltzmann hierarchy, recovering the acoustic-peak power spectrum, polarization spectra, and lensing reconstruction. The empirical leakage parameter $\varepsilon$ in the bound above is the magnitude of (ii); the structural prediction $\varepsilon = 0$ follows if the cross-term vanishes identically. The required machinery (mode-projected propagators, anti-periodic boundary conditions, hierarchy-sensitive transfer functions) is expressible within the framework's existing postulates, but the calculation has not been carried through. The §§4-6 predictions tested in this paper do not depend on this derivation: the CMB consistency stands at the structural-argument-plus-empirical-bound level, and the formal perturbation-theory proof is deferred to separate future work.
...
**Sensitivity bracketing.** The representative $(M_\text{halo}, c)$ values above sit in the middle of plausible 1σ uncertainty bands for Moster-style SHMR and Duffy-style concentration evolution at the $L^*$ archetype. To bracket the ΛCDM prediction width against the framework's mass- and aperture-universal point prediction, the same NFW + Newtonian-inversion pipeline is evaluated under two additional parameterizations: a *pessimistic* set ($\sim+33\%$ on $M_\text{halo}$, $\sim+20\%$ on $c$ relative to representative) chosen to maximize the ΛCDM $M_\text{dyn}/M_b$ at $R = 100$ kpc and thereby narrow the discriminator; and an *optimistic* set ($\sim-33\%$ on $M_\text{halo}$, $\sim-20\%$ on $c$) chosen to flatten the ΛCDM redshift trend.

**Table B.1.** ΛCDM $L^*$ $M_\text{dyn}/M_b$ at $R = 100$ kpc under three SHMR + concentration parameterizations, against the framework's mass- and aperture-universal point prediction. "Pessimistic" / "optimistic" are in the discriminator-strength sense: pessimistic = ΛCDM tracks the framework most closely; optimistic = ΛCDM is flattest.

| Parameterization | $(M_\text{halo}, c)$ at $z = 0$ | $(M_\text{halo}, c)$ at $z = 1$ | $(M_\text{halo}, c)$ at $z = 2$ | $M_\text{dyn}/M_b$: $z=0$ / $z=1$ / $z=2$ |
|---|---|---|---|---|
| Pessimistic | $(2.0 \times 10^{12}, 9.0)$ | $(1.4 \times 10^{12}, 6.0)$ | $(1.0 \times 10^{12}, 4.5)$ | 17.7 / 17.4 / 17.5 |
| Representative | $(1.5 \times 10^{12}, 7.5)$ | $(1.0 \times 10^{12}, 5.0)$ | $(7.0 \times 10^{11}, 3.5)$ | 14.0 / 13.6 / 13.8 |
| Optimistic | $(1.0 \times 10^{12}, 6.0)$ | $(7.0 \times 10^{11}, 4.0)$ | $(5.0 \times 10^{11}, 2.5)$ | 10.3 / 10.5 / 11.2 |
| **Framework prediction (universal)** | — | — | — | **11.98 / 16.03 / 20.86** |

The discriminator (framework / ΛCDM at $z = 2$) under the three parameterizations is 1.19 (pessimistic), 1.52 (representative), and 1.86 (optimistic). Even under the pessimistic parameterization that pushes ΛCDM as close to the framework as plausible 1σ choices allow, the framework's $z = 2$ point prediction sits 19% above the ΛCDM upper bracket — distinguishable in a stratified Euclid DR1 stack at the few-percent statistical sensitivity expected (§6.6). The mass- and aperture-dependence of the ΛCDM shift persists across the bracket; the framework's universal $\sqrt{E(z)}$ enhancement, point-valued and parameter-free at fixed $a_0(0)$, is the falsifiable signature.
