...
This also clarifies the role of measured inputs. The framework predicts structural relations: the integer floors $n=1,2,3$, the McKay exponents, the grid fractions, the well assignments, and the dimensionless ratios between observables sharing a calibrated sector. It does not claim that every absolute normalization is derived without empirical reference. A measured anchor per sector is part of the calibration architecture. In the mass sector specifically, the absolute floor is the vacuum scale $\mu_\Lambda = \rho_\Lambda^{1/4}$ inherited from the surface sector; $m_e$ is the mass-sector normalization that fixes the convention tying the predicted ratios to laboratory units, not a second floor competing with $\mu_\Lambda$.
