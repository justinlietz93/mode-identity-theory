...
This is below the precision of current BAO measurements (DESI DR2: ~2–3% per bin in D_H/r_d) but potentially within reach of the next generation. Euclid DR1 spectroscopic BAO (z = 0.9–1.8, four redshift bins) is forecast at 1–2% per bin [16], marginal for detection. The discriminating regime is DESI full-survey and Euclid DR2 at ~0.5% per bin, where the (1+z)¹ signature reaches approximately 1.5σ per bin for s₀ ~ 0.15, with correlated bins improving aggregate sensitivity. A definitive test of the full s₀ > 0.10 range requires next-generation spectroscopic surveys (MegaMapper, MUST, WST) forecast at sub-0.5% per-bin precision [17].
