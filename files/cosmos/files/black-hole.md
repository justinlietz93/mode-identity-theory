...
At $s = 0$, the Pochhammer symbol $(0)_k = 0$ for $k \geq 1$ eliminates every term with $k \geq 1$, leaving $Z_{0,\sigma}(0) = \widetilde{H}_0(0)$ exactly: the tower collapses to its single surviving term, it does not by itself force that survivor to vanish. For the nontrivial twists under consideration here, $\widetilde H_0(0)$ itself vanishes, and the two facts together, tower collapse and a vanishing survivor, give $Z_\sigma(0) = 0$. This is a tower collapse followed by a vanishing survivor, not one mechanism producing the zero on its own, and not two independent zeros.
