...
**The one load-bearing assumption.** Step B uses homothety-covariance of the self-adjoint extension: it is what guarantees the boundary profile sin(πΘ) is itself R-independent. Friedrichs, the extension already in use, is natural and therefore covariant in the abstract. The open question is whether the smoothed-cone limit that selects it is taken covariantly: whether the smoothing scale ε enters only as the dimensionless ratio ε/R (removed in the limit) rather than as an absolute length. *If* the limit is covariant, the separation is forced and the factorization and extension-selection items are not independent: the same covariant limit closes both, collapsing two open mechanisms toward one. If ε enters as an absolute scale, the boundary profile can acquire R-dependence and a cross-term can return. So the ε/R question is the single bottleneck for both items, and it is the eigenvalue paper's call, not settled here. This is why the status stays CANDIDATE PROOF, not forced.
