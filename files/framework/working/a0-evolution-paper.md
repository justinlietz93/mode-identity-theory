...
First, the relation is *derived* from the framework's scaling-law postulate (§2) as a zero-parameter cancellation between two specific Fibonacci-well positions $C(13/120)$ and $C(34/120)$ on the 120-domain of $S^3/2I$, with the local Milgrom ratio recovered at the 0.8% level. The derivation identifies the precise topological mechanism by which the proportionality arises and forces a single specific functional form (linear in $E(z)$) rather than admitting a family of speculative possibilities.
...
One relation. Five independent tests. Zero free parameters for the redshift evolution beyond the local SPARC calibration. The framework's evolution claim stands or falls on the next two years of intermediate-redshift kinematic and lensing data.
