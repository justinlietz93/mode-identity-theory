...
**Status (form forced):** $A/A_P = C(\Theta) \cdot (\sqrt{\Omega})^{-n}$ began as a declared measurement postulate. The form is now forced in all three rows: the anti-periodic BC forces $C(\Theta)$ (first-positive mode intensity, under the named harmonic selection isotropy + orthogonality), units force the integer-depth hierarchy $(\sqrt{\Omega})^{-n}$ on the dilution sector, and the Schur separation (Lemma 8 + homothety reduction) forces the factored form, for parity-preserving extensions with $\delta_0/R > 2/e$ (the Friedrichs default included). The Gleason-type promotion from declared to forced is thus achieved for the form. What remains is off the form: the $\alpha_W$ twist mechanism, the self-adjoint extension *selection* (governs only the ground state), and the edge/surface scale identification $\Omega_H = \Omega_\Lambda$ (an observed coincidence).
