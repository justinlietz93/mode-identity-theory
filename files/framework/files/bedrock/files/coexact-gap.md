...
The distinguished status of $`\Sigma(2,3,5) = S^3/2I`$ is familiar in gauge theory: among spherical space forms with nontrivial fundamental group it is the unique integral homology sphere, the spherical example singled out by Fintushel and Stern [FintushelStern] in the instanton-Floer setting, where spectral-flow computations with adjoint-bundle coefficients appear in Nasatyr [Nasatyr1992] and in Kirk and Klassen [KirkKlassen]. Helle records the character tables, Dynkin diagrams and flat-connection organization for the binary polyhedral spaces [Helle]. On the spectral side, untwisted form spectra are computed directly in related settings, namely the Hodge Laplacian on $`1`$-forms of homogeneous $`3`$-spheres [HenkelLauret2026] and the $`p`$-form spectra of lens spaces [LauretLens2018]. Henkel and Lauret vary the metric on a single $`3`$-sphere across the Berger and left-invariant families, so their coexact bottom varies with the metric. Here the round metric is fixed and the flat connection varies across the ADE family, so the adjoint gap is constant but for the single jump at the $`2I`$ Galois connection. Twisted spectra also have a precedent: Lauret [LauretCyclic] computes the spectrum of the Laplacian on functions twisted by the characters of a cyclic fundamental group, those characters being its one-dimensional irreducibles. The present note treats coexact $`1`$-forms instead, and in the adjoint application the twists are the adjoint representations $`\mathrm{Sym}^2 \rho`$ of the two-dimensional irreducible flat $`\mathrm{SU}(2)`$ connections carried by the nonabelian groups of the ADE classification. Thus the two results are complementary.

On the coexact $`1`$-form summand the operator is the square of the curl-type first-order operator $`\ast d_\nabla`$, so the gap records the square of the bottom absolute coexact curl eigenvalue rather than spectral flow. In the bipartite principal case $`d_\tau \ge 2`$, that eigenvalue is $`d_\tau/R`$. The bottom eigenvalue is read from the classical restricted form spectrum together with the McKay correspondence, so the value $`36/R^2`$ rests on classical analysis; the spectral-flow and eta-invariant literature does not single it out as a coexact spectral gap. The gap is nonetheless gauge-theoretic, though as a static quantity rather than a flow: in the adjoint case the coexact summand is the gauge-fixed part of the linearized flat-connection problem, the exact forms are the infinitesimal gauge directions, and the coclosed condition is the Coulomb slice. Thus the coexact gap is the first positive spectral stiffness of the flat connection in that slice, equivalently the bottom of the slice Laplacian at one connection rather than an invariant of a path.
