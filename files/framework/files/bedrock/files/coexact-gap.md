...
Every irreducible flat $`\mathrm{SU}(2)`$ connection on a quotient $`S^3/\Gamma`$ with $`\Gamma \subset \mathrm{SU}(2)`$ thus has adjoint coexact gap $`4/R^2`$, with the single exception of the Galois connection on $`S^3/2I`$, whose gap is $`36/R^2 = 6^2/R^2`$. The factor $`9`$ between them is the square of the distance ratio $`6/2`$ on the affine $`E_8`$ graph.
