...
**Relation to the literature.** The physical lineage is point interactions and quantum mechanics on cones. The bridging condition is a Kirchhoff-type vertex condition [BK]; its closest predecessor is the length-parametrized cone-apex extension of Kay and Studer [KayStuder], whose logarithmic bound-state branch is the analogue of ours, without the Möbius topology or the width transition. Through the bridging family the model realizes the two-dimensional one-center point interaction of [AGHH] at a conical point of a curved non-orientable surface. The deformed tower this produces is an instance of the rank-one secular mechanism of Colin de Verdière's pseudo-Laplacians [CdV], here with the interlacing towers explicit. The model thus combines two standard sources of low-energy spectral structure, a global flat holonomy and a local point interaction: the holonomy organizes the transverse parity sectors, the point interaction controls the ground-state sector. On the applied side, quantum dynamics on Möbius geometries is an active mesoscopic topic, from energy levels and optical transitions on Möbius rings [LiRM] to electronic states in Möbius quantum wires [PSA]; the present model supplies the exactly solvable singular end of that family.
...
> **Remark 3.4** (Aharonov-Bohm flux analogy)**.** In Aharonov-Bohm language the wide-band effect is the geometric analogue of a flux line. The $\pi$ holonomy shifts the effective angular momenta by half-integers, and widening the band drags the lowest shifted momentum below one, the range in which a two-dimensional flux line is famously ambiguous (limit-circle) at the origin.
...
> **Remark 3.6** (Scattering dictionary)**.** In scattering language, sector-regularity confines the point interaction to the s-wave (constant angular) channel. Every other channel, including the wide-band first odd one, is held at the regular branch.
