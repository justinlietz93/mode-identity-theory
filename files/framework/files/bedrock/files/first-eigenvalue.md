...
We study the twisted Laplacian, the Laplace-Beltrami operator on sections of the orientation line bundle, on a constant-curvature Möbius band $M(W)$ of half-width $W$, built from a spherical band on $S^2(R)$ with the antipodal-edge identification. Its metric collapses one transverse fiber to a cone point, where the operator is not essentially self-adjoint in the constant transverse sector, so the bottom of the spectrum depends on the self-adjoint extension. For the Friedrichs extension the bottom is $0$, attained by a discontinuous piecewise-constant zero mode $\phi_0$, and the operator is unitarily equivalent to the Neumann Laplacian on a spherical lune. The sector-regular bridging realization excludes $\phi_0$ and instead carries a single cone-localized negative eigenvalue $\lambda_b(\delta_0)$, set by a renormalization length $\delta_0$, with small-$\delta_0$ asymptotic $\lambda_b(\delta_0) = -4e^{-2\gamma}/\delta_0^2\,(1 + O(\delta_0^2/R^2))$; by Friedrichs maximality, no self-adjoint extension has strictly positive spectral bottom. 

The first positive eigenvalue, by contrast, is stable across these realizations: for the Friedrichs extension and for bridging with $\delta_0 > 2R/e$ it equals $2/R^2$ for $0 < W \le \pi R/2$ and $\alpha_0(\alpha_0+1)/R^2$, $\alpha_0 = \pi R/(2W)$, for $\pi R/2 < W < \pi R$, doubly degenerate at the critical width. We prove this by separating the operator into transverse Neumann sectors and solving the reduced singular Sturm-Liouville problems in closed form via Legendre functions. For $W > \pi R/2$ the explicit representative of the first positive eigenfunction changes sign, reflecting the nontriviality of the orientation bundle.

---

## 1. Introduction

On a Möbius band the orientation line bundle has nontrivial holonomy: after one circuit of the core loop a section changes sign. On a smooth non-orientable surface a continuous nowhere-vanishing section of the orientation line bundle would trivialize it, so the usual scalar Perron-Frobenius ground-state mechanism is unavailable for the twisted Laplacian (cf. [Shigekawa] for the magnetic spectral-positivity criterion). This paper asks how that topological obstruction is expressed when the metric also has a conic degeneration.

In the constant-curvature model considered here one transverse fiber collapses to a cone point, and there the twisted Laplacian is not essentially self-adjoint, so the bottom of the spectrum is not determined by the formal differential expression alone. The two effects act on different data: the twist is a sign condition on eigenfunctions, while the location of the bottom is settled by the extension theory at the cone. The discontinuous piecewise-constant section $\phi_0$ is a genuine zero mode of the Friedrichs extension, and Friedrichs maximality then bounds the bottom of every self-adjoint extension from above by $0$: nonnegative extensions attain this bound, while the bridging family replaces $\phi_0$ by a logarithmic bound state below zero. The first positive eigenvalue is more rigid: it is common to the Friedrichs extension and to every bridging realization above an explicit threshold of the renormalization length, and it undergoes a computable transition at the critical width $W = \pi R/2$.

The precise setting is the twisted Laplacian, the Laplace-Beltrami operator on sections of the orientation line bundle, on the constant-curvature Möbius band $M(W)$ constructed from a spherical band on $S^2(R)$ of half-width $W$, with metric $ds^2 = dy^2 + \cos^2(y/R)\,dw^2$, cone point $p_c$ at $y = \pi R/2$, and Neumann conditions on the regular boundary arcs. Both singular structures are intrinsic to the model rather than inserted by hand: the twist is the holonomy of the orientation bundle, and the cone is produced by the metric itself, the meridians focusing at the pole of the covering sphere. Constant curvature keeps the model exactly solvable, every reduced one-dimensional problem below being Legendre, so the competition between bundle topology and conic extension theory can be followed in closed form, from the operator domain down to the individual eigenfunctions. At $p_c$ the operator is not essentially self-adjoint in the constant transverse sector (the transverse-mode subspaces of §3.4), and it is there that the choice of extension is made. In the wide-band regime $W > \pi R/2$ the first odd nonconstant sector is also limit-circle at the cone; in the Friedrichs and bridging realizations considered here that sector is fixed by the regular Frobenius branch (Definition 3.4). Equivalently, $M(W)$ may be viewed as a spherical lune whose two vertices are identified, or as the antipodal quotient of a double lune; this viewpoint is developed in Remark 2.1 and explains why the cone conditions can either decouple or couple the two vertex traces.
...
> **Theorem 1.1** (Classification of the bottom of the spectrum)**.** Let $A$ be any self-adjoint extension at the cone of the twisted Laplacian on $M(W)$, $0 < W < \pi R$. Then $\inf\sigma(A) \le 0$. Equality holds if and only if $A$ is a nonnegative extension; among these the Friedrichs extension is maximal, and its bottom is attained by the discontinuous piecewise-constant zero mode $\phi_0$ (eigenvalue $0$, the constant-sector kernel). The bridging realization $A^{\mathrm{br}}_{\delta_0}$ (Definition 3.4) is not nonnegative: it carries a single cone-localized negative eigenvalue $\lambda_b(\delta_0)$, with small-$\delta_0$ asymptotic $\lambda_b(\delta_0) = -4e^{-2\gamma}/\delta_0^2\,(1 + O(\delta_0^2/R^2))$, $\gamma$ the Euler-Mascheroni constant, and zero lies in the resolvent set. In particular no self-adjoint extension has strictly positive spectral bottom.
...
>
> At $W = \pi R/2$ it is doubly degenerate. Both branches are realized by explicit eigenfunctions: $\sin(y/R)$, the $\ell = 1$ zonal Legendre polynomial, for the narrow regime, and $\lvert\cos(y/R)\rvert^{\alpha_0}\sin(\pi w/2W)$, whose profile in $y$ is $(1-x^2)^{\alpha_0/2}$ in the variable $x = \sin(y/R)$, reducing to the sectoral harmonic $P_{\alpha_0}^{\alpha_0}$ at integer degree, for the wide regime.
...
The proof separates the operator into transverse Neumann sectors and solves each reduced Sturm-Liouville problem in closed form via Legendre functions. Only the constant (zonal) sector carries the cone-condition ambiguity that separates our realizations: cone parity splits it into a symmetric tower, common to the Friedrichs and bridging realizations, and an antisymmetric tower, where they differ. The narrow-regime first positive value $2/R^2$ comes from the symmetric tower and is robust; the zero or negative bottom comes from the antisymmetric tower, where the bridging condition deforms the Friedrichs zero mode through an explicit secular function and pushes the bottom below zero (§4).

Cheeger [Cheeger1979, Cheeger1983] founded the spectral geometry of conic singularities, with the Friedrichs extension and its link-geometry deficiency indices. Boscain and Prandi [BP] classify the self-adjoint extensions of the model conic metric $ds^2 = dx^2 + \lvert x\rvert^{-2\alpha_{\mathrm{BP}}}\,d\theta^2$ in the cone regime $\alpha_{\mathrm{BP}} \in (-3, -1]$, which contains our $\alpha_{\mathrm{BP}} = -1$, distinguishing disjoint (Friedrichs) and bridging dynamics. The local asymptotics are regular-singular in the sense of Brüning and Seeley [BS], and the limit-point/limit-circle alternative is governed by the inverse-square theory of Kalf, Schmincke, Walter, and Wüst [KSWW, RS]. The bridging condition is a Kirchhoff-type vertex condition [BK]; its closest predecessor is the length-parametrized cone-apex extension of Kay and Studer [KayStuder], whose logarithmic bound-state branch is the analogue of ours, though without the Möbius topology or the width transition. Through the bridging family the model realizes the two-dimensional one-center point interaction of [AGHH] at a conical point of a curved non-orientable surface, with the renormalization length $\delta_0$ in its standard role; the deformed tower this produces is an instance of the rank-one secular mechanism of Colin de Verdière's pseudo-Laplacians [CdV], here with the interlacing towers computed in closed form. The anti-periodic half-integer longitudinal quantization is the line-bundle holonomy spectrum of Ballmann, Brüning, and Carron [BBC]. The novelty claimed here is accordingly not a new extension theory or a new secular method, but the exact assembly of these classical ingredients in the orientation-bundle Möbius geometry, where the extension-dependent bottom and the extension-stable first positive eigenvalue are computed simultaneously and in closed form.

The closest Möbius-strip predecessor is Kalvoda, Krejčiřík, and Zahradová [KKZ], whose thin-strip asymptotics for the Dirichlet Laplacian on a Möbius strip in $\mathbb{R}^3$ yield effective one-dimensional operators. The decisive difference is our Neumann boundary condition, which supplies the constant transverse mode that makes the cone a limit-circle endpoint and creates the extension ambiguity; on the curved, cone-bearing model the result is an exact finite-width spectral calculation rather than a thin-strip asymptotic. The two models are, moreover, not asymptotic reflections of one another: the effective potential of [KKZ] is sourced by the geodesic curvature of a non-geodesic core curve, whereas our core loop is a geodesic and the degeneration survives at every width as the cone, so the thin limits differ structurally (§7). What the models share is the parity bookkeeping of the Möbius holonomy, on opposite branches: their surviving Dirichlet mode, acting on scalar functions, is longitudinally periodic, while our Neumann constant mode, acting on sections of the orientation bundle, is anti-periodic.
...
Neither finite Dirichlet energy nor semiboundedness alone singles out a realization in the wide band: the constant-sector logarithmic mode of §4.4 has infinite Dirichlet energy, and by Lemma 3.3 the singular-branch odd extensions are semibounded as well. We therefore work only with realizations that select the regular (Frobenius) branch in every nonconstant sector, the Friedrichs choice there, and we do not treat realizations that couple distinct transverse sectors. The two we single out, the Friedrichs and bridging realizations, are defined in §3.5 (Definition 3.4).
...
**Sector-bottom divergence.** The sector bottoms tend to $+\infty$, which is what gives each realization of Definition 3.4 compact resolvent and discrete spectrum (Proposition 3.5). In the sector with transverse Neumann eigenvalue $\mu$ the indicial exponent is $\alpha = \sqrt{\mu}\,R$, and the ground-state transforms of §6.1 and §6.2 identify the nonconstant sector bottom as $\alpha(\alpha+1)/R^2$. In the bridging realization the constant sector is bounded below by $\lambda_b(\delta_0)$ (Proposition 4.4), while in the Friedrichs realization it is bounded below by $0$; in either case the constant sector is a single summand and does not affect the divergence of the nonconstant sector bottoms. The transverse Neumann eigenvalues $\mu_k^{\mathrm{e}} = (k\pi/W)^2$ and $\mu_j^{\mathrm{o}} = ((2j+1)\pi/(2W))^2$ increase without bound in the sector index, so $\alpha \to \infty$ and the bottoms $\alpha(\alpha+1)/R^2 \to +\infty$.
...
The renormalization length $\delta_0$ is the one quantity in the model that the intrinsic geometry leaves free. The Friedrichs extension needs no such scale and is the standard conic choice [Cheeger1979, Cheeger1983]; its spectrum is the Neumann lune spectrum of Proposition 4.2, so it does not encode any additional vertex-matching data from the Möbius identification, and the bottom is the zero mode. At $\alpha_{\mathrm{BP}} = -1$ it is moreover the unique Markovian realization (Theorem 1.13(ii) of [BP]), the bridging extensions being non-Markovian, and together with the generic decoupling of two-dimensional point interactions this makes Friedrichs the canonical nonnegative choice; a transmitting limit would survive only if a smoothing of the cone were tuned to a zero-energy resonance. Whether a geometric regularization selects a canonical $\delta_0$, for instance as a norm-resolvent limit of smoothed-cone Laplacians [AGHH], remains open. Until then $\lambda_b$ is an extension-dependent quantity rather than an intrinsic invariant, and its leading form carries $O(\delta_0^2/R^2)$ corrections that are not small at moderate $\delta_0$.

The thin-strip limit is already visible in the closed forms, and it separates this model from the effective dynamics of [KKZ] rather than connecting to them. As $W \to 0$ every nonconstant sector bottom $\alpha(\alpha+1)/R^2$ diverges, the smallest indicial exponent being $\alpha_0 = \pi R/(2W)$, so on any fixed spectral window the spectrum reduces to the $W$-independent constant sector, the zonal problem of §4: for the Friedrichs extension the combined towers $\ell(\ell+1)/R^2$, $\ell \ge 0$, and for bridging the symmetric tower together with the deformed antisymmetric roots and the bound state $\lambda_b(\delta_0)$. The limit is thus the zonal Legendre problem rather than a Mathieu-type effective operator as in [KKZ]: their effective potential is generated by the geodesic curvature of a non-geodesic core curve, while our core loop is a geodesic and the degeneration is concentrated in the cone at every width. What the two models share is the parity bookkeeping of the Möbius holonomy, and the two thin limits lie on opposite branches of it: their surviving Dirichlet transverse mode, on scalar functions, forces periodic longitudinal behavior, while our Neumann constant mode, on sections of the orientation bundle, forces anti-periodic behavior (§3.4). Natural continuations within the same self-adjoint framework include Robin conditions at the regular boundary arcs and the wider family of Lagrangian cone-trace conditions of §3.5.
