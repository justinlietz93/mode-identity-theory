...
**Status (2026-07-01):** Open problem, no theorem yet. The two bedrock results sit on the two pieces of the postulate $S^1 = \partial(\text{Möbius}) \hookrightarrow S^3$, $\partial S^3 = \emptyset$, and whether a theorem connects them through the topology of the embedding is open. The naive bridge through the shared value $2/R^2$ is dead: it is a forced curvature-scale coincidence, not a spectral link. The one live route is index theory on the $E_8$ plumbing; Steps 1-3 are done (the boundary gatekeeper is open; the analytic framework and orientation are fixed; the interior classes are computed, and the mod-2/mod-4 homology turns out to be Galois-blind, so the Galois selection is carried by the McKay/tautological decoration alone), and the make-or-break coupling is Step 4, revised by Step 3 from a two-ingredient to a three-ingredient test; its bookkeeping (part one) is closed, and the coupling identity itself is open. This note tracks the routes and where each stands.
