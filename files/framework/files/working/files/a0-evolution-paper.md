...
**Surface modes ($n = 2$, $\Omega_\Lambda$).** The Möbius strip carries an intrinsic eigenvalue: the first positive mode of the Laplace-Beltrami operator on the totally geodesic surface in $S^3$ gives $\lambda_1 = 2/R^2$, where $R$ is the curvature radius of $S^3$ (the constant zero mode is the extension-dependent background, not this eigenvalue). The Gauss-Codazzi embedding converts this to $\Lambda_\text{obs} = (3/2)\lambda_1 = 3/R^2$ under three conditions: totally geodesic embedding ($K_{ij} = 0$), isotropy (CMB-verified to $10^{-5}$), and de Sitter vacuum. The hierarchy ratio $\Omega_\Lambda = (\Lambda\ell_P^2)^{-1}$ is set by this eigenvalue and does not evolve.
