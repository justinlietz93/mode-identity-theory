...
**Measurement update (2026-07-05, post-run).** Gate (ii) remains conditionally closed as a theorem of the registered premises. The measurement track challenges P2: the observable asymmetry dial excludes $\epsilon = 0$ at 95% in the frozen run ([registration Results](clock-asymmetry-fit.md)). The half-power clock is therefore not promoted to DERIVED; it remains a motivated symmetric-kernel branch, while the measured family selects a small negative asymmetry (tick exponent $\approx 1.39$ to $1.41$ at 68%). Diagnosis, not repair, is the adopted posture; the diagnostic sequence lives on the registration page.
...
| P2 | kernel exchange symmetry, equivalently $\epsilon = 0$ in the asymmetry family of §III: the elementary rule carries no arrow; direction lives in the state | SUPPORTED, reduced by [the chase](sampling-kernel-symmetry.md): P2 follows from R1 (magnitude-only registration, prior doctrine) and R2 (state-free rate rule, structural premise). **MEASURED 2026-07-05: row 3 of the [registration](clock-asymmetry-fit.md) fired: zero excluded at 95% in both tiers ($\hat\epsilon = -0.106$, $2.8\sigma$ over the symmetric point). The reduction stands as mathematics; by modus tollens the data challenge R1 or R2, or the dial is contaminated (the preferred valley rides the $s_0$ degeneracy). P2's empirical status is downgraded; the lemma is not. The diagnostic sequence on the registration page runs before any R1/R2 verdict.** |
