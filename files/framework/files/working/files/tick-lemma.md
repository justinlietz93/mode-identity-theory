...
| P2 | kernel exchange symmetry, equivalently $\epsilon = 0$ in the asymmetry family of §III: the elementary rule carries no arrow; direction lives in the state | SUPPORTED (entropy §IX, through the time-reversal bridge of Step 2); the single load-bearing non-theorem premise; two upgrade paths below |
...
**The theorem path for P2 (the hermiticity chase; open).** Self-adjointness of a generator equalizes forward and reverse matrix elements in magnitude, which is P2 at the level of the elementary rule. The framework already owns proved self-adjoint objects: the [first-eigenvalue pillar](../../bedrock/files/first-eigenvalue.md)'s realizations of the twisted Laplacian, the Friedrichs extension and the sector-regular bridging family (no self-adjoint extension has a positive bottom; the first positive level is $2/R^2$). The chase question, sharp and open: is the sampling kernel generated by one of these, or by any self-adjoint object already in service? A yes moves P2 from SUPPORTED to DERIVED, and the whole chain becomes one definitional premise plus theorems.

**The measurement path for P2 (registered, pending).** Give the kernel an asymmetry $\epsilon$: flux $\propto w_1^{1/2-\epsilon} w_2^{1/2+\epsilon}$, still degree one, grade $3/2 + \epsilon$; then $dT \propto S^{3/2+\epsilon}\,dt$ and $dt/d\tau = S^{-1/2-\epsilon}$. Every clock model already run is an $\epsilon$ slice:
