...
| R-HALF | the self-dual clock under level exchange. Two faces of one route: the amplitude-level lapse $N^2 = S$, and the geometric mean of the two level measures, $d\tau = \sqrt{dt \cdot S\,dt} = S^{1/2}\,dt$. The faces are identical: the geometric mean of the phase measure and the intensity measure *is* the lapse $N^2 = S$. | $S^{-1/2}$ exactly | LANDS arithmetically; the one landing route, and it carries the whole derivation burden through a three-step gate. (i) *Involution existence.* In the lapse convention $d\tau = S^\alpha\,dt$, the exchange $\alpha \leftrightarrow 1 - \alpha$ swaps the phase measure ($\alpha = 0$) and the intensity measure ($\alpha = 1$) and has the unique fixed point $\alpha = 1/2$. Physically this is level exchange, counting at amplitude level versus intensity level, the same $\psi$ vs $\lvert\psi\rvert^2$ split that separates the 120-grid from the 60-grid. The question is whether the exchange exists as an operator, not just on exponents. If yes, self-duality under level exchange is the mechanism and the $\sqrt{\Omega}$ midpoint becomes a corollary; if no, both faces lose their principle at once. One question, two rows settled. (ii) *The tick lemma, the decider.* Define the observer tick as a sampling functional on the realized temporal amplitude and prove whether the induced lapse satisfies $N^2 = S$, $N^2 = S^2$, or neither. This decides the route regardless of how (i) turns out. (iii) *Second consequence, named before scoring.* $N^2 = S$ means geometry couples at amplitude level while observables live at intensity level ($g_{tt} = -S$ against the phase metric $-dt^2$), and that commitment must produce something beyond the exponent it was built around, with the candidate sector named in advance: perturbation-level propagation or the sound-horizon bookkeeping. A route that returns only its design target is decorated, not derived. |
...
**Order of attack.** R-HALF's gate first, in its own order: the involution existence check is the cheapest sharp question on the board and settles whether the self-duality story is available; the tick lemma is the decider for the route either way; the second consequence gets named before any scoring. Lemma 0.1 second: cheap, worth writing regardless, and only ever a dictionary upgrade. Then R-COUNT, whose lemma is shared with the entropy page and pays double. R-TWIN enumeration last, as the systematic sweep. R-STAB is not attacked; it is fenced. Ranking: R-HALF, then R-COUNT, then R-TWIN, with R-STAB fenced.
