...
**Order of attack.** Gate (i) is done: the involution exists at measure level and the self-duality story is available ([The Level Exchange](half-power-involution.md)). Next is the tick lemma in its sharpened form, the decider; the second consequence gets committed there, with the two candidate sectors already named (the R-COUNT fusion with the entropy page's mode map, and any perturbation-sector fingerprint surviving the EFE-unchanged constraint). Lemma 0.1 stays queued: cheap, worth writing regardless, and only ever a dictionary upgrade. Then R-COUNT in its own right, then the R-TWIN sweep. R-STAB is not attacked; it is fenced. Ranking: R-HALF, then R-COUNT, then R-TWIN, with R-STAB fenced.
