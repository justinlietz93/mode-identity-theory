...
**Order of attack.** Gates (i) and (ii) are done, gate (ii) conditionally on its premise ledger ([The Level Exchange](half-power-involution.md), [The Tick Lemma](tick-lemma.md)). Gate (iii)'s first candidate is committed on the [entropy page](entropy-as-realization-budget.md), and the completing fit ($dt/d\tau = S^{-3/2}$ in the conformal seat, expected dead at leading $(1+z)^5$) is registered for the Λcos pipeline. Next: review of the tick lemma's premise ledger, then the armed channels score the result. Lemma 0.1 stays queued: cheap, worth writing regardless, and only ever a dictionary upgrade. Then R-COUNT in its own right, then the R-TWIN sweep. R-STAB is not attacked; it is fenced. Ranking: R-HALF, then R-COUNT, then R-TWIN, with R-STAB fenced.
