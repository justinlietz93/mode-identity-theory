...
**Order of attack.** Gate (i) is done: the involution exists at measure level and the self-duality story is available ([The Level Exchange](half-power-involution.md)). Gate (iii)'s first candidate is committed on the [entropy page](entropy-as-realization-budget.md), and the completing fit ($dt/d\tau = S^{-3/2}$ in the conformal seat, expected dead at leading $(1+z)^5$) is registered for the Λcos pipeline. Next is the tick lemma in its two-property form, the decider. Lemma 0.1 stays queued: cheap, worth writing regardless, and only ever a dictionary upgrade. Then R-COUNT in its own right, then the R-TWIN sweep. R-STAB is not attacked; it is fenced. Ranking: R-HALF, then R-COUNT, then R-TWIN, with R-STAB fenced.
