...
| Waltz clock map t(Φ); entropy S↦N_max; energy E(S) (the amplitude-to-$T_{\mu\nu}$ dictionary; one object with the cooling-energy accounting per the [Budget Map](budget-map.md), redshift-and-cooling §VI + entropy §VIII) | load-bearing | see self-inconsistencies below |
| Charm assignment; ν₂ gap; dead-zone coverage | soft | known fermions the scheme cannot place |
| energy-as-resolution, entropy-as-realization-budget | non-load-bearing | speculative derivation candidates with unwalked steps. Redshift-and-cooling is not a separate count: its redshift and cooling kinematics are established, and its one open piece (§VI) is the E(S) dictionary in the load-bearing row above |
