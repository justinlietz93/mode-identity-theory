...
| 3 | Dilution (√Ω)^−n / factorization | any dilution law; factored vs additive/mixed forms | **dimensionally forced + unresolved** — the integer-depth power is units; separating it from C(Θ) rides the commutant theorem, an open problem two rows up | Λ, H₀/a₀, dark-null |
...
| 5 | Exponent: Coxeter (13,17), 1/\|grid\| | 47-combo restricted class; 3,527-combo broad control | **chosen, audited** — exceptional in the restricted class (1 hit vs ~0.08 expected; alt Coxeter pairs 93–770% off), **null in the broad control (3 hits ≈ 3 expected)** | couplings |
| 6 | a₀/(cH₀) pair (13,34) | 7,021 phase-position pairs, ±1% of the observed ratio | **chosen, audited (conditional)** — exceptional within the registered well class (24 pairs = 0.34% within 1%; (13,34) is 1 of the 6 Fibonacci pairs); denominator not shown pre-target; a₀ and H₀ are inputs, the ratio a retrodiction | a₀/(cH₀) |
...
**What the four tests do and do not pay.** They quantify the freedom inside four named rule classes; they do not discharge the theory-level selection of the classes themselves. A test pays the look-elsewhere cost only to the extent its admissible set, statistic, and tolerance were fixed independently of the target. Two were: the wells sweep enumerated a natural space and mass-null-v1.0 was pre-registered, and both came back NULL. The other two are the positives, and their denominators (the restricted 47, the six Fibonacci pairs) were developed alongside the framework, not demonstrably before the target. **Verdict: rows 5 and 6 read as provisional, not established, until those denominators are shown to be fixed independent of the answer.** The broad α scan (3,527 combinations, 3 hits ≈ 3 expected) makes this concrete: the entire positive content lives in the restriction, which is exactly where the post-hoc exposure lives.
