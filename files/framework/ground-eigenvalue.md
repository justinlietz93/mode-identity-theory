...
**Lemma (Neumann unfolding).** After the change of angular variable $\theta = w/R$, even reflection at the Neumann endpoints identifies each transverse Neumann eigensector on $[-W, W]$ with a Fourier eigensector on the circle link of angular circumference $4W/R$ (unitarily, up to the harmless normalization factor coming from reflection). For each transverse sector, the reduced one-dimensional operator in $y$ has the same singular endpoint at $\delta = 0$, the same weight $|\delta|/R$, and the same indicial equation. Since the maximal domain, Lagrange bracket, and deficiency indices depend only on the local behavior at the singular endpoint [Weidmann], the Boscain-Prandi extension classification [BP] transfers from the circle-link model cone to the Neumann half-cone: the Friedrichs and transmission extensions are characterized by the same boundary data $(u_D^\pm, u_N^\pm)$ in both settings.
...
The exponent $-2\alpha - 1 < -1$ for every $\alpha > 0$, so this divergence holds throughout the limit-circle regime. The Friedrichs extension excludes the singular branch by requiring finite Dirichlet integral, selecting the regular solution $|\delta|^{+\alpha}$. The regular eigenfunction $|\delta|^{\alpha} = |\cos(y/R)|^{\alpha}$ vanishes at the cone; continuity there is automatic. The Friedrichs extension and the continuous transmission extension coincide. (The function $|\cos(y/R)|^\alpha = (1-x^2)^{\alpha/2}$ for non-integer $\alpha \in (0,1)$ is the regular solution of the associated Legendre equation at degree and order $\alpha$; it reduces to $P_\alpha^\alpha$ at integer $\alpha$. Its properties are developed in §5.)

The limit-point/limit-circle classification at a singular endpoint depends only on the local behavior of the coefficients [Weidmann]. Near $p_c$, the weight $|f|$ vanishes linearly in $\delta$, matching the flat-cone case $\alpha = -1$ in the notation of Boscain and Prandi [BP]. Their local criterion transfers: each sector's classification is determined by its indicial exponent $\alpha = \sqrt{\mu}\,R$, with limit-point for $\alpha \geq 1$ and limit-circle for $\alpha < 1$. On BP's model cone (angular period $2\pi$), only $k = 0$ has $\alpha < 1$. On the Möbius band, the unfolded period $4W/R$ produces fractional exponents, so for wide bands ($W > \pi R/2$) the first odd sector also enters the limit-circle regime. The extension ambiguity nonetheless remains confined to the constant sector: in nonconstant limit-circle sectors, the singular branch has infinite Dirichlet energy (§3.4 above), so the Friedrichs and transmission extensions coincide.
...
In §5, the odd sectors will admit non-integer associated Legendre solutions because the transverse potential shifts the indicial exponents away from zero, opening a fractional-power regular branch.
...
on $(0, \pi R)$ with periodic boundary conditions $u(\pi R) = u(0)$, $u'(\pi R) = u'(0)$. In standard Schrödinger form, the effective potential is proportional to $\sec^2(y/R)$, a symmetric Pöschl-Teller potential; the exact solvability of §5.2 reflects this. The transmission condition at the cone (§3.5) is continuity plus matching of weighted flux; for the regular Frobenius branch at $\alpha_j > 0$, the Friedrichs and transmission extensions coincide (§3.4), and the eigenfunction vanishes at the cone.
...
### 5.3 Identification as an associated Legendre solution

Under the substitution $x = \sin(y/R)$ of §4.2, the function $|\cos(y/R)|^{\alpha} = (1 - x^2)^{\alpha/2}$ is the regular solution at $x = 1$ (the cone) of the associated Legendre equation with degree and order both equal to $\alpha$. For integer $\alpha$, this is proportional to the classical sectoral harmonic $P_\alpha^\alpha(x)$; for non-integer $\alpha$, the standard Ferrers function $P_\alpha^\alpha$ diverges at $x = 1$ as $(1-x)^{-\alpha/2}$, so the regular branch $(1-x^2)^{\alpha/2}$ must be distinguished from it. At the transition $\alpha = 1$, the eigenfunction reduces to $Y_1^1$ on the covering $S^2$.

Two ingredients explain why non-integer $\alpha$ is admissible here but excluded in §4. First, the indicial exponents: in the constant sector (§4), both exponents were zero ($s^2 = 0$), and the singular $Q_\nu$ branch diverged logarithmically. In the odd sectors, the exponents are $s = \pm\alpha$ with $\alpha > 0$; the regular branch $|\delta|^{+\alpha}$ generates $(1-x^2)^{\alpha/2}$, and no $Q$-type singularity arises. Second, the boundary condition: the anti-periodic condition $P_\nu(0) = 0$ in §4 forced $\nu$ to odd integers. The periodic condition $u(\pi R) = u(0)$ imposes no constraint on $\alpha$, since $|\cos(y/R)|^\alpha$ evaluates to $1$ at both seams for all $\alpha$. The spectral parameter runs continuously, set by the transverse width $W$ through $\alpha = (2j+1)\pi R/(2W)$.
...
At the critical width, the odd-sector eigenfunction becomes $|\cos(y/R)|\,\sin(w/R)$. The absolute value is required by the equivariance condition: the smooth harmonic $\cos(y/R)\sin(w/R)$ on $S^2$ is $\tau$-invariant and descends to a scalar function, not a section of $\mathcal{L}$, while $|\cos(y/R)|\sin(w/R)$ is $\tau$-anti-invariant and satisfies $\psi(\pi R, -w) = -\psi(0, w)$. Its eigenvalue $2/R^2$ coincides with the zonal branch $Y_1^0 \propto \sin(y/R)$: two members of the $\ell = 1$ multiplet meeting at the transition.
...
*Proof.* Because the metric coefficient $\cos^2(y/R)$ multiplying $\psi_{ww}$ depends on $y$ alone, the twisted Laplacian commutes with the transverse Neumann Laplacian on $[-W, W]$, and $L^2(M, dA)$ decomposes as a Hilbert-space direct sum $\bigoplus_n L^2\bigl((0,\pi R),\,|f|\,dy\bigr) \otimes \mathrm{span}(\Phi_n)$ over the transverse eigensectors. The transmission conditions (continuity and flux matching at the cone) are stated in terms of one-sided limits at $\delta = 0$ and depend only on the longitudinal function $u(y)$; they are imposed independently in each sector, so the transmission extension respects the decomposition. The resolvent is compact by the standard compactness for regular-singular elliptic operators on a compact conic surface with fixed self-adjoint extension, so the spectrum is discrete and the global ground eigenvalue equals the minimum of the sector ground eigenvalues. By the sector ground states of §5.8:
...
On the smooth covering $S^2$, $\psi_1$ restricts from $Y_1^0$; $\psi_2$ does not lift to a smooth function on $S^2$ (the absolute value is required by the equivariance condition, not a coordinate artifact). Away from $W = \pi R/2$, the ground eigenvalue is simple.
...
The sign-changing ground state in the wide regime deserves comment. On an orientable surface, the Perron-Frobenius theorem and Courant's nodal domain theorem guarantee a positive ground eigenfunction. On $M(W)$, both arguments are unavailable: a nowhere-vanishing section would trivialize $\mathcal{L}$, so every continuous section must vanish somewhere, and no positivity cone is preserved by the semigroup. This is an instance of Shigekawa's result [Shigekawa] that the ground state of a magnetic Laplacian must possess zeros when the holonomy is nontrivial. The two regimes have different nodal structures: for $W > \pi R/2$, the ground eigenfunction changes sign across $w = 0$ (the core loop); for $W < \pi R/2$, the eigenfunction $\sin(y/R)$ is positive on the interior of the covering rectangle but vanishes at the seam $y = 0 \sim y = \pi R$, where the Möbius identification imposes the sign reversal.
...
- [Shigekawa] I. Shigekawa. Eigenvalue problems for the Schrödinger operator with the magnetic field on a compact Riemannian manifold. *J. Funct. Anal.* 75 (1987), 92--127.
