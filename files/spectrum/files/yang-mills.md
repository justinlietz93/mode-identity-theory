...
shows the first nontrivial invariant at $N = 12$. Five even-degree shells ($N = 2, 4, 6, 8, 10$) are empty. The mechanism parallels the gauge sector: the McKay recursion walks along the $E_8$ graph, and the trivial representation does not reappear until the walk returns after twelve steps. This shell gap is a spectral fact about the scalar sector; its cosmological reading as a low-ℓ CMB mode deficit is held in abeyance, since matching the observed deficit scale would require a curvature radius the data exclude (see [CMB Anomalies](../../cosmos/files/cmb-anomalies.md)).
